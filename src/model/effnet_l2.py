...
class EfficientNetL2(nn.Module):
    def __init__(self, freeze_pretrained=False, weight_init=False):
        super().__init__()
        # Use EfficientNet-B7 with pretrained weights
        ...
        if isinstance(first_block, nn.Sequential):
            conv = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False)
            if weight_init:
        ...
        # Optionally freeze everything except first conv and classifier
