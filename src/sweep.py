...
def main():
    ...
    parser.add_argument(
        "--sweep_id", type=str, default=None, help="Existing W&B sweep ID"
    )
    parser.add_argument(
        "--sweep_config", type=str, default="sweep_resnet", help="Existing W&B sweep ID"
    )
    ...
    if args.sweep_id:
        sweep_id = (
            f"{settings.WANDB_TEAM_NAME}/{settings.WANDB_PROJECT_NAME}/{args.sweep_id}"
        )
    else:
